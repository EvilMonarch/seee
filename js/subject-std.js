  ...
  doc.docs
    ...
    .forEach(data => {
      if (data.data().namecourse != 'วิชา4') {
        num = num+1;
        $('#subject-std').append(createSubject(data.data().namecourse,data.data().nameteacher, data.data().room, data.data().time,data.data().passcourse,data.data().day,num,data.data().teacher)
        )
      }
    })
})
...
function createSubject(namecourse, nameteacher, room, time,passcourse,day, num,teacher) {
  ...
  html += '<a onclick="ChooseFunction(2,'+num+')"><div class="information information_btn" name="post" value="2">ระบบลา</div></a> '
  html += '</div></div></div>'
...
function ChooseFunction(num, id){
  ...
  localStorage.setItem("teacher", document.getElementById("teacher_"+id).value)
  if(num == 1){
    window.location.href = "check2.html";
  }
  if(num == 2){
    window.location.href = "check3.html";
  }
