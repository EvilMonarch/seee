...
db.collection('teacher').doc(localStorage.getItem('id')).collection('subject').get().then(snapshot => {
    snapshot.docs.forEach(doc => {
        if (namecourse == doc.data().namecourse) {
            if (week == "week01") {
                var num = doc.data().week01.length
                for (i = 1; i < num; i++) {
                    if(doc.data().week01[i]!='STOP'){
